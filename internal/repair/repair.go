...
func (g *Generator) suggestedNextAction(report guard.Report, failures []Failure) string {
	...
	if len(failures) == 0 {
		return "Inspect the failing guard steps, then rerun `fkn repair` after making a targeted fix."
	}
	first := failures[0]
	if first.Scope != nil {
		return fmt.Sprintf("Start with `%s` in scope `%s`, fix the reported errors, then rerun `fkn repair %s`.", first.Task, first.Scope.Name, report.Guard)
	}
	return fmt.Sprintf("Start with `%s`, fix the reported errors, then rerun `fkn repair %s`.", first.Task, report.Guard)
}

func render(report guard.Report, failures []Failure, gitDiff, suggested string) string {
	var sections []string
	sections = append(sections, "# fkn repair")
