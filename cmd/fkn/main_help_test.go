...
func TestRunHelpForTask(t *testing.T) {
	...
	if !strings.Contains(output, "Usage: fkn check [--dry-run] [--allow-unsafe] [--json]") {
...
`), 0o644); err != nil {
	...
	if !strings.Contains(output, "Usage: fkn b [--dry-run] [--allow-unsafe] [--json]") {
...
`), 0o644); err != nil {
	...
	output := readStdout()
	for _, want := range []string{"Usage: fkn build <target> [<files...>] [--dry-run] [--allow-unsafe] [--json]", "positional[1]", "positional[2...]"} {
		if !strings.Contains(output, want) {
			t.Fatalf("stdout = %q, want %q", output, want)
		}
	}
